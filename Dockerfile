...
#
# Only tagging needs ANTHROPIC_API_KEY; fetching and the test suite do not.
...
# The databases and the fetched images live on volumes, not in the image:
# a container that is thrown away must not take the run history with it.
ENV RESULTS_DB=/data/results.sqlite3 \
    MEMORY_DB=/data/.agent_memory.sqlite3

# Streamlit writes to $HOME; a non-root user needs one it owns.
